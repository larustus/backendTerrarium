...
public interface PinRepository extends CrudRepository<Pin, Integer>{
    ...
    @Query("SELECT p FROM Pin p WHERE p.terrarium_id = :terrarium_id AND p.user.id = :user_id")
    List<Pin> findByTerrariumId(@Param("terrarium_id") Integer terrarium_id, @Param("user_id") Integer user_id);
}
